    ...
    var methods = {
        find: function (locator, el, timeout) {
            function waitReturnElement(){
                ...
                }, timeout || timeouts.wait, "[drivex.find] Element not found for locator " +
                showLocator(locator))
            }
        ...
        /**
         * wraps Selenium FindElements
         * @param locator {LocatorJSON}
         * @param el {WebElement}
         * @returns {Promise} resolve to the element list for the locator
         */
        list: function (locator, el) {
            return methods.finds(locator, el)
        },
        ...
        click: function (locator, el) {
            return methods.find(locator, el).then(async function (elt) {
                await driver.wait(wd.until.elementIsVisible(elt), timeouts.wait, "element should be visible to be clicked");
                await methods.delay(timeouts.delay)
                return elt.click()
            })
        },
        ...
        jsClick: function (locator, el) {
         ...
         /**
         * wraps function of check an element
         * @param locator {LocatorJSON}
         * @param el {WebElement}
         * @returns {Promise} resolved or rejected
         */
        check: function (locator, el) {
            return methods.find(locator, el).then(async function (elt) {
                await elt.getTagName().then(tag => {
                    assert.equal(tag.toLowerCase(),"input", "check/select element should only work for input")
                })
                if(!await elt.isSelected()){
                    return driver.executeScript("arguments[0].click();", elt)
                }
        ...
        fill: function (locator, el, value) {
            return methods.find(locator, el).then(async function (elt) {
                await driver.wait(wd.until.elementIsVisible(elt), timeouts.wait, "element should be visible while set values");
        ...
        waitForElementPromise: function (locator, timeout, msg) {
            function waitReturnElement() {
                return methods.waitForElement(locator, timeout || timeouts.wait, msg);
            }
        ...
        waitForElementVisiblePromise: function (locator, timeout, msg) {
            function waitVisibleReturnElement() {
                return methods.waitForElementVisible(locator, timeout || timeouts.wait, msg);
            }
        ...
        waitForCssValue: async function (locator, timeout, expected, msg) {
            ...
            await methods.waitForElement(locator, timeout || timeouts.wait, msg).then(async ele => {
                try{
                    await driver.wait(async function(){
         ...
         * @param locator {LocatorJSON}
         * @param timeout {Number}
         * @param expected {expected condition of WebElement}
         * @param msg {String} optional message for any error messages
         * @returns {WebElementPromise} resolves to WebElement or throw error
         */
        waitFor: async function (locator, timeout, expected, msg) {
            ...
            if(["invisible","notVisible"].includes(expected)){
                ...
                await methods.waitForElement(locator, timeout, msg + expected).then(async ele => {
        ...
        is: async function (locator, parentWebElement, expected, msg) {
            if(["invisible","notVisible"].includes(expected)){
                ...
                .then(async function(eles){
                    if(eles.length > 0 && (await eles[0].isDisplayed().catch(() => {return false}))){
        ...
        delay: async function(timeout = 500){
